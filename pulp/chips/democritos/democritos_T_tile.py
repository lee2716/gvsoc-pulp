...
class Democritos_T_TileTcdm(gvsoc.systree.Component):
    def __init__(self, parent, name, parser):
        ...
        # 1 master: RedMulE
        redmule_masters = 1
        redmule_interleaver = HWPEInterleaver(self, "redmule_interleaver", nb_master_ports=redmule_masters, nb_banks=nb_banks, bank_width=DemocritosArch.BYTES_PER_WORD)
...
class Democritos_T_Tile(gvsoc.systree.Component):
    def __init__(self, parent, name, parser, tid: int=0):
        ...
        # Data scratchpad
        l1_tcdm = Democritos_T_TileTcdm(self, f'tile-{tid}-l1-tcdm', parser)
       ...
       # AXI and OBI x-bars
        ...
        # RedMulE, configured the way magia_v2/tile.py:199 configures it so the
        # two platforms model the same accelerator: an 8x8 array reached through
        # a 32-byte port. elem_size is the widest element the buffers have to
        # hold, not the format in use, so FP8 operands do not shrink it.
        ...
        self.add_properties({
            "event_unit": {
                ...
                "config": {
                    "nb_core": 1,                      # T-tile has one CV32 core
                    "properties": {
                        "dispatch":  {"size": 8},
                        "mutex":     {"nb_mutexes": 0},
        ...
        # Event unit: map it on the OBI xbar and route the RedMulE completion
        ...
        # RedMulE completion -> event 10, the slot magia_v2 uses for it.
