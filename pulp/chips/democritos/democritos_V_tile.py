...
class Democritos_V_TileTcdm(gvsoc.systree.Component):
    def __init__(self, parent, name, parser):
        ...
        spatz_interleaver = L1_interleaver(self, 'spatz_interleaver', nb_slaves=nb_banks,
            nb_masters=spatz_masters, interleaving_bits=2,
            offset_mask=DemocritosArch.L1_TILE_OFFSET-1)
...
class Democritos_V_Tile(gvsoc.systree.Component):
    def __init__(self, parent, name, parser, tid: int=0, rom_file: str=None):
        ...
        # Data scratchpad
        l1_tcdm = Democritos_V_TileTcdm(self, f'tile-{tid}-l1-tcdm', parser)
       ...
       # AXI and OBI x-bars
        ...
        spatz = SnitchFast(self, f'tile-{tid}-snitch-spatz',
            ...
            inc_spatz=True,
            vlen=DemocritosArch.SPATZ_VLEN,
            spatz_nb_lanes=DemocritosArch.SPATZ_NB_LANES,
            spatz_lane_width=DemocritosArch.SPATZ_LANE_WIDTH,
            pulp_v2=False, ssr=False)
        ...
        # Wide inbound traffic is another tile's DMA writing this L1, so it
        # takes the DMA interleaver rather than the core-side one.
    ...
    # Output (master) wide port to off-tile L2 memory. DMA traffic only; core
    # accesses use the narrow port.
