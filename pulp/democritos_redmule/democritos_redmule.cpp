 ...
 *
 *      Also UINT8 x UINT8 into UINT32 (compute_able 8, ARITH 0x500): simulation only,
 *      the RTL has no integer format.
 */
...
void matmul_uint8_uint32(uint32_t * z, uint32_t * y, uint8_t * x, uint8_t * w, uint16_t m_size, uint16_t n_size, uint16_t k_size);
...
class DemocritosRedmule : public vp::Component
{

public:
    DemocritosRedmule(vp::ComponentConf &config);
...
// private: ? why not private?????
    ...
    uint32_t next_addr();
    uint32_t calculate_tile_base_address(uint32_t base, uint32_t stride, uint32_t tile_col, uint32_t tile_row, uint32_t i, uint32_t j, uint32_t elem);
    uint32_t inc_addr(uint32_t addr, uint32_t stride, uint32_t tile_row, uint32_t elem);
    void     set_format(uint32_t compute_able);
    ...
    uint32_t            elem_size;
    uint32_t            in_elem;        // bytes per X and W element
    uint32_t            out_elem;       // bytes per Y and Z element
    uint32_t            out_accesses;   // TCDM requests per Y or Z row
    uint32_t            y_sub;          // request index within the current Y row
    uint32_t            z_sub;          // request index within the current Z row
    uint32_t            sub_off;        // row offset of the current request
    uint32_t            sub_size;       // bytes carried by the current request
    uint32_t            ce_height;
    uint32_t            ce_width;
    uint32_t            ce_pipe;
...
{
    return new DemocritosRedmule(config);
}

DemocritosRedmule::DemocritosRedmule(vp::ComponentConf &config)
    : vp::Component(config)
{
    //Initialize interface
    this->traces.new_trace("trace", &this->trace, vp::DEBUG);
    this->input_itf.set_req_meth(&DemocritosRedmule::req);
    this->new_slave_port("input", &this->input_itf);
    this->input_itf_v2.set_req_meth(&DemocritosRedmule::req_v2);
    ...
    this->offload_itf.set_sync_meth(&DemocritosRedmule::offload_sync);
    this->new_slave_port("offload", &this->offload_itf, this);

    // Declare offload master interface for granting blocked transfers
    //this->offload_grant_itf.set_sync_meth(&DemocritosRedmule::offload_grant);
    ...
    this->LOCAL_BUFFER_N    = this->bandwidth;  // widest N: one-byte operands
    this->in_elem           = this->elem_size;
    this->out_elem          = this->elem_size;
    this->out_accesses      = 1;
    this->y_sub             = 0;
    this->z_sub             = 0;
    this->sub_off           = 0;
    ...
    this->y_buffer_preload  = new uint8_t [this->LOCAL_BUFFER_H * this->LOCAL_BUFFER_W * 4];
    this->w_buffer          = new uint8_t [this->LOCAL_BUFFER_N * this->LOCAL_BUFFER_W];
    this->x_buffer          = new uint8_t [this->LOCAL_BUFFER_H * this->LOCAL_BUFFER_N];
    this->z_buffer_compute  = new uint8_t [this->LOCAL_BUFFER_H * this->LOCAL_BUFFER_W * 4];
    this->z_buffer_previos  = new uint8_t [this->LOCAL_BUFFER_H * this->LOCAL_BUFFER_W * 4];

    //Initialize FSM
    this->state.set(IDLE);
    this->redmule_query     = NULL;
    this->tcdm_req          = this->tcdm_itf.req_new(0, 0, 0, 0);
    this->fsm_event         = this->event_new(&DemocritosRedmule::fsm_handler);
    ...
    this->trace.msg("[DemocritosRedmule] Model Initialization Done!\n");
}

void DemocritosRedmule::init_redmule_meta_data(){
    uint32_t buffer_h = this->ce_height;
    uint32_t buffer_w = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n = this->bandwidth / this->in_elem;
...
uint32_t DemocritosRedmule::next_iteration(){
    this->iter_k += 1;
...
uint32_t DemocritosRedmule::calculate_tile_base_address(uint32_t base, uint32_t stride, uint32_t tile_col, uint32_t tile_row, uint32_t i, uint32_t j, uint32_t elem){
    /*
    *       ----
    *   col |  |
    *       ----
    ...
    */
    if (this->fold_tiles_mapping)
    {
        ...
        return base + (i * tiles_per_row + j) * tile_row * tile_col * elem;
    } else {
        return base + (i * tile_col * stride + j * tile_row) * elem;
    }
}

uint32_t DemocritosRedmule::inc_addr(uint32_t addr, uint32_t stride, uint32_t tile_row, uint32_t elem){
    if (this->fold_tiles_mapping)
    {
        return addr + tile_row * elem;
    } else {
        return addr + stride * elem;
    }
}

uint32_t DemocritosRedmule::tmp_next_addr(){
    this->z_addr = (this->z_addr + this->bandwidth) % (this->ce_height * this->bandwidth);
    return this->z_addr;
}

uint32_t DemocritosRedmule::next_addr(){
    uint32_t addr       = 0;
    uint32_t buffer_h   = this->ce_height;
    uint32_t buffer_w   = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n   = this->bandwidth / this->in_elem;
    ...
    {
        // A row wider than the port goes out in out_accesses requests.
        uint32_t row_bytes = buffer_w * this->out_elem;
        this->sub_off  = this->y_sub * this->bandwidth;
        uint32_t left  = row_bytes > this->sub_off ? row_bytes - this->sub_off : 0;
        this->sub_size = left < this->bandwidth ? left : this->bandwidth;
        addr = this->iter_y_addr + this->sub_off;
        this->y_sub += 1;
        if (this->y_sub == this->out_accesses)
        {
            this->y_sub = 0;
            this->iter_y_addr = this->inc_addr(this->iter_y_addr, this->k_size, buffer_w, this->out_elem);
        }
        this->y_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][Address] Y tile at 0x%11x | #Y tile left %d\n", addr, this->y_acc_block);
        this->tcdm_req->set_is_write(0);
        this->tcdm_req->set_size(this->sub_size ? this->sub_size : this->out_elem);
        this->iter_instruction  = INSTR_LOAD_Y;
    } else if (this->w_acc_block > 0)
    {
        addr = this->iter_w_addr;
        this->iter_w_addr = this->inc_addr(addr, this->k_size, buffer_w, this->in_elem);
        this->w_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][Address] W tile at 0x%11x | #W tile left %d\n", addr, this->w_acc_block);
        this->tcdm_req->set_is_write(0);
        this->tcdm_req->set_size(buffer_w * this->in_elem);
    ...
    } else if (this->x_acc_block > 0)
    {
        ...
        this->iter_x_addr = this->inc_addr(addr, this->n_size, buffer_n, this->in_elem);
        this->x_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][Address] X tile at 0x%11x | #X tile left %d\n", addr, this->x_acc_block);
        this->tcdm_req->set_is_write(0);
        this->tcdm_req->set_size(this->bandwidth);
        this->iter_instruction  = INSTR_LOAD_X;
    } else if (this->z_acc_block > 0)
    {
        // Same split as Y; an empty request past a short row is a read.
        uint32_t row_bytes = this->z_store_width * this->out_elem;
        this->sub_off  = this->z_sub * this->bandwidth;
        uint32_t left  = row_bytes > this->sub_off ? row_bytes - this->sub_off : 0;
        this->sub_size = left < this->bandwidth ? left : this->bandwidth;
        addr = this->iter_z_addr + this->sub_off;
        this->z_sub += 1;
        if (this->z_sub == this->out_accesses)
        {
            this->z_sub = 0;
            this->iter_z_addr = this->inc_addr(this->iter_z_addr, this->k_size, buffer_w, this->out_elem);
        }
        this->z_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][Address] Z tile at 0x%11x | #Z tile left %d\n", addr, this->z_acc_block);
        this->tcdm_req->set_is_write(this->sub_size ? 1 : 0);
        this->tcdm_req->set_size(this->sub_size ? this->sub_size : this->out_elem);
        this->iter_instruction  = INSTR_STOR_Z;
    } else {
        this->trace.fatal("[DemocritosRedmule][Address] INVALID redmule address iteration : No tiles to access\n");
    }

    return addr;
}

void DemocritosRedmule::process_compute(){
    uint32_t buffer_h           = this->ce_height;
    uint32_t buffer_w           = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n           = this->bandwidth / this->in_elem;
    ...
    if (this->compute_able == 8)
    {
        //UINT8 x UINT8 into UINT32
        matmul_uint8_uint32((uint32_t *)this->z_buffer_compute,
                        (uint32_t *)this->z_buffer_compute,
...
void DemocritosRedmule::process_iter_instruction(){
    ...
    */
    ...
    uint32_t buffer_h_byte = this->ce_height * this->out_elem;
    uint32_t buffer_n_byte = this->bandwidth;
    uint32_t buffer_w_byte = this->ce_width * (this->ce_pipe + 1) * this->in_elem;       // one W row
    uint32_t buffer_wo_byte = this->ce_width * (this->ce_pipe + 1) * this->out_elem;     // one Y or Z row
    ...
    uint32_t x_leftover_byte = this->n_size * this->in_elem - _k * buffer_n_byte;
    uint32_t x_cutoff = x_leftover_byte < buffer_n_byte ? x_leftover_byte : buffer_n_byte;

    uint32_t w_leftover_byte = this->k_size * this->in_elem - this->iter_j * buffer_w_byte;
    ...
    uint32_t y_leftover_byte = this->k_size * this->out_elem - _j * buffer_wo_byte;
    uint32_t y_cutoff = y_leftover_byte < buffer_wo_byte ? y_leftover_byte : buffer_wo_byte;

    uint32_t z_width_leftover_byte = this->k_size * this->out_elem - this->iter_j * buffer_wo_byte;
    uint32_t z_width_cutoff = z_width_leftover_byte < buffer_wo_byte ? z_width_leftover_byte : buffer_wo_byte;
    uint32_t z_height_leftover_byte = this->m_size * this->out_elem - this->iter_i * buffer_h_byte;
    uint32_t z_height_cutoff = z_height_leftover_byte < buffer_h_byte ? z_height_leftover_byte : buffer_h_byte;

    uint32_t buffer_yz_byte = this->ce_height * this->ce_width * (this->ce_pipe + 1) * this->out_elem;
    switch(this->iter_instruction) {
        case INSTR_LOAD_Y: {
            if (this->iter_y_row_ptr == 0 && this->sub_off == 0) {
                std::memset(this->y_buffer_preload, 0, this->LOCAL_BUFFER_H * this->LOCAL_BUFFER_W * this->out_elem);
            }
            uint32_t n = this->sub_off < y_cutoff ? y_cutoff - this->sub_off : 0;
            if (n > this->sub_size) n = this->sub_size;
            std::memcpy(&(this->y_buffer_preload[this->iter_y_row_ptr + this->sub_off]), this->access_buffer, n);
            if (this->y_sub == 0)   // last request of the row
            {
                if (this->y_acc_block == 0)
                {
                    this->iter_y_row_ptr = 0;
                } else {
                    this->iter_y_row_ptr += buffer_wo_byte;
                }
            }
            break;
        }
        case INSTR_LOAD_W:
            if (this->iter_w_row_ptr == 0) {
                std::memset(this->w_buffer, 0, this->LOCAL_BUFFER_N * this->LOCAL_BUFFER_W);
            }
        ...
        case INSTR_LOAD_X:
            ...
            break;
        case INSTR_STOR_Z:
            std::memcpy(this->access_buffer, &(this->z_buffer_previos[this->iter_z_row_ptr + this->sub_off]), this->sub_size);
            if (this->z_sub == 0)   // last request of the row
            {
                if (this->z_acc_block == 0)
                {
                    this->iter_z_row_ptr = 0;
                } else {
                    this->iter_z_row_ptr += buffer_wo_byte;
                }
            }
            break;
        case INSTR_FORWARD_YZ:
            ...
            this->z_store_width = z_width_cutoff / this->out_elem;
            this->z_store_height = z_height_cutoff / this->out_elem;
            break;
        default:
            break;
    }
...
uint32_t DemocritosRedmule::get_routine_access_block_number(){
    ...
    this->iter_w_addr = this->calculate_tile_base_address(this->w_addr, this->k_size, buffer_n, buffer_w, this->iter_k, this->iter_j, this->in_elem);
    ...
    if (is_last_iteration == 0)
    {
        ...
        this->iter_x_addr = this->calculate_tile_base_address(this->x_addr, this->n_size, buffer_h, buffer_n, _i, _k, this->in_elem);
    ...
    if (this->iter_k == (this->x_row_tiles - 1) && (is_last_iteration == 0))
    {
        ...
        this->iter_y_addr = this->calculate_tile_base_address(this->y_addr, this->k_size, buffer_h, buffer_w, _i, _j, this->out_elem);

        this->y_acc_block = ((this->m_size - _i * this->ce_height) < this->ce_height ? (this->m_size - _i * this->ce_height) : this->ce_height) * this->out_accesses;
    ...
    {
        this->z_acc_block = this->z_store_height * this->out_accesses;
        ...
        this->iter_z_addr = this->calculate_tile_base_address(this->z_addr, this->k_size, buffer_h, buffer_w, _i, _j, this->out_elem);
    }
...
uint32_t DemocritosRedmule::get_preload_access_block_number(){
    uint32_t total_blocks       = 0;
    uint32_t tcdms_bw           = this->bandwidth / this->in_elem;
    ...
    this->y_acc_block = (this->m_size < this->ce_height ?  this->m_size : this->ce_height) * this->out_accesses;
...
uint32_t DemocritosRedmule::get_storing_access_block_number(){
    uint32_t total_blocks       = 0;
    uint32_t buffer_h           = this->ce_height;
    uint32_t buffer_w           = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n           = this->bandwidth / this->in_elem;
    ...
    this->z_acc_block = this->z_store_height * this->out_accesses;
    total_blocks = this->z_acc_block;
    this->iter_z_addr = this->calculate_tile_base_address(this->z_addr, this->k_size, buffer_h, buffer_w, this->z_col_tiles - 1, this->z_row_tiles - 1, this->out_elem);

    return total_blocks;

}

uint32_t DemocritosRedmule::get_routine_to_storing_latency(){
    return this->ce_width * (this->ce_pipe + 1);
    // return 0;
}

uint32_t DemocritosRedmule::get_redmule_array_runtime(){
    uint32_t tcdms_bw       = this->bandwidth / this->in_elem;
...
uint32_t DemocritosRedmule::op_foramt_parser(uint32_t op_format) {
    ...
    if ((operation==1) && (data_format==1))
        ...
        compute_able=7;
    else if ((operation==1) && (data_format==2))
        compute_able=8;     // UINT8 x UINT8 into UINT32
    else 
        this->trace.fatal("[DemocritosRedmule] Selected wrong operation/format combination [op_format=0x%x-data_format=%d-operation=%d]\n",op_format,data_format,operation);
    return compute_able;
}

void DemocritosRedmule::offload_sync(vp::Block *__this, IssOffloadInsn<uint32_t> *insn)
{
    DemocritosRedmule *_this = (DemocritosRedmule *)__this;
    ...
    switch (opc)
    {
        case 0b0001011:
        {
            if (_this->state.get() == IDLE) {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set MNK size: %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
            }
            break;
        }
        case 0b0101011:
        {
            if (_this->state.get() == IDLE)
            {
                ...
                _this->set_format(_this->compute_able);
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set XWY addr: 0x%08x, 0x%08x, 0x%08x\n", _this->x_addr, _this->w_addr, _this->y_addr);

                /*************************
                *  Asynchronize Trigger  *
                *************************/
                //Sanity Check
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] redmule configuration (M-N-K): %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
                if ((_this->m_size == 0)||(_this->n_size == 0)||(_this->k_size == 0))
                {
                    _this->trace.fatal("[DemocritosRedmule] INVALID redmule configuration (M-N-K): %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
                }
...
vp::IoReqStatus DemocritosRedmule::req(vp::Block *__this, vp::IoReq *req)
{
    DemocritosRedmule *_this = (DemocritosRedmule *)__this;
    ...
    //_this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] access (offset: 0x%x, size: 0x%x, is_write: %d, data:%x)\n", offset, size, is_write, *(uint32_t *)data);
        ...
        switch (offset) {
            case 0x00: {
                if ((_this->redmule_query == NULL) && (_this->state.get() == IDLE)) {
                    ...
                    ************************/
                    //Sanity Check
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] redmule configuration (M-N-K): %d, %d, %d. Compute able is %d\n", _this->m_size, _this->n_size, _this->k_size,_this->compute_able);
                    if ((_this->m_size == 0)||(_this->n_size == 0)||(_this->k_size == 0))
                    {
                        _this->trace.fatal("[DemocritosRedmule] INVALID redmule configuration (M-N-K): %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
                        return vp::IO_REQ_OK;
            ...
            case 0x40: {
                _this->x_addr = value;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set X addr 0x%x)\n", _this->x_addr);
                break;
            }
            case 0x44: {
                _this->w_addr = value;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set W addr 0x%x)\n", _this->w_addr);
                break;
            }
            case 0x48: {
                _this->y_addr = value;
                _this->z_addr = _this->y_addr;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set Y addr 0x%x, Set Z addr 0x%x)\n", _this->y_addr,_this->z_addr);
                break;
            }
            case 0x4C: { //REDMULE_MCFG0_PTR
                _this->m_size=value & 0xFFFF;
                _this->k_size=value >> 16;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set M size %d, Set K size %d)\n", _this->m_size,_this->k_size);
                break;
            }
            case 0x50: { //REDMULE_MCFG1_PTR
                _this->n_size=value & 0xFFFF;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set N size %d)\n", _this->n_size);
                break;
            }
            case 0x54: {
                _this->compute_able = _this->op_foramt_parser((value == 0x480)? 9 : (value == 0x500)? 0xA : 0); //the marith mapping between reg-if and offload-if is different... WHY?
                _this->set_format(_this->compute_able);
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] marith 0x%x, compute_able 0x%x, elem_size %d)\n", value,_this->compute_able,_this->elem_size);
                break;
            }
            default:
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] write to INVALID address\n");
        }
    }
    else {
        switch (offset) {
            case 0x00:
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] read to INVALID address\n");
                break;
            case 0x0C: {
                int32_t done_id_r;
                if ((_this->redmule_query == NULL) && (_this->state.get() == IDLE)) {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] read status reg 0x%x\n",done_id_r);
                break;
            }
            default:
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] read to INVALID address\n");
...
vp::IoReqStatus DemocritosRedmule::req_v2(vp::Block *__this, vp::IoReq *req)
{
    DemocritosRedmule *_this = (DemocritosRedmule *)__this;
    ...
    if (is_write == 1) {
        ...
        switch (offset) {
            ...
            case 0x20: { //REDMULE_MCFG0_PTR
                _this->m_size=value & 0xFFFF;
                _this->k_size=value >> 16;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set mcfg_reg0 -- M size %d, Set K size %d)\n", _this->m_size,_this->k_size);
                break;
            }
            case 0x24: { //REDMULE_MCFG1_PTR
                _this->n_size=value & 0xFFFF;
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set mcfg_reg1 -- N size %d)\n", _this->n_size);
                break;
            }
            case 0x28: { //REDMULE_MCFG2_PTR
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule] Set mcfg_reg2 -- currently not used\n");
                break;
            }
            case 0x2C: {
                _this->x_addr = value;
...
vp::IoReqStatus DemocritosRedmule::send_tcdm_req()
{
    return this->tcdm_itf.req(this->tcdm_req);
}

void DemocritosRedmule::fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    DemocritosRedmule *_this = (DemocritosRedmule *)__this;
    ...
    switch (_this->state.get()) {
        ...
        case PRELOAD: {
            ...
            if ((_this->fsm_counter < _this->tcdm_block_total) && (_this->pending_req_queue.size() <= _this->queue_depth))
            {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][Preload] --- Send TCDM req #%d [addr=0x%08x]\n",_this->fsm_counter,temp_addr);

                //Check error
                if (err != vp::IO_REQ_OK) {
                    _this->trace.fatal("[DemocritosRedmule][Preload] There was an error while reading/writing data\n");
                    return;
                }
            ...
            while((_this->pending_req_queue.size()!= 0) && (_this->pending_req_queue.front() <= _this->fsm_timestamp)){
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][Preload] ---                           Receive TCDM resp\n");
                // std::string out;
        ...
        case ROUTINE: {
            ...
            if ((_this->fsm_counter < _this->tcdm_block_total) && (_this->pending_req_queue.size() <= _this->queue_depth))
            {
                ...
                //Send request
                vp::IoReqStatus err = _this->send_tcdm_req();
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][ROUTINE-ijk: %0d-%0d-%0d] --- Send TCDM req #%d [addr=0x%08x]\n", _this->iter_i, _this->iter_j, _this->iter_k, _this->fsm_counter,temp_addr);

                //Check error
                if (err != vp::IO_REQ_OK) {
                    _this->trace.fatal("[DemocritosRedmule][ROUTINE-ijk: %0d-%0d-%0d] There was an error while reading/writing data\n", _this->iter_i, _this->iter_j, _this->iter_k);
                    return;
                }
            ...
            //Recieve Process
            while((_this->pending_req_queue.size()!= 0) && (_this->pending_req_queue.front() <= _this->fsm_timestamp) ){
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][ROUTINE-ijk: %0d-%0d-%0d] ---                           Receive TCDM resp\n", _this->iter_i, _this->iter_j, _this->iter_k);
                // std::string out;
                // for (int i = 0; i < _this->tcdm_req->get_size(); i += 2) {
                //     char tmp[16];
                //     snprintf(tmp, sizeof(tmp), "0x%02x%02x, ", _this->access_buffer[i + 1], _this->access_buffer[i]);
            ...
            if ((_this->fsm_counter >= _this->tcdm_block_total) && (_this->pending_req_queue.size() == 0))
            {
                ...
                {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][ROUTINE-ijk: %0d-%0d-%0d] TCDM Access Time(%d) > Model Time(%d)\n", _this->iter_i, _this->iter_j, _this->iter_k, _this->fsm_timestamp, modeled_runtime);
                    latency = 1;
                } else {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][ROUTINE-ijk: %0d-%0d-%0d] Model Time(%d) > TCDM Access Time(%d)\n",  _this->iter_i, _this->iter_j, _this->iter_k, modeled_runtime, _this->fsm_timestamp);
        ...
        case STORING: {
            ...
            if ((_this->fsm_counter < _this->tcdm_block_total) && (_this->pending_req_queue.size() <= _this->queue_depth))
            {
                ...
                //Send request
                vp::IoReqStatus err = _this->send_tcdm_req();
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][Storing] --- Send TCDM req #%d [addr=0x%08x]\n",_this->fsm_counter,temp_addr);

                //Check error
                if (err != vp::IO_REQ_OK) {
                    _this->trace.fatal("[DemocritosRedmule][Storing] There was an error while reading/writing data\n");
                    return;
                }
            ...
            //Recieve Process
            while((_this->pending_req_queue.size()!= 0) && (_this->pending_req_queue.front() <= _this->fsm_timestamp) ){
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][Storing] ---                           Receive TCDM resp\n");
                // std::string out;
                // for (int i = 0; i < _this->tcdm_req->get_size(); i += 2) {
                //     char tmp[16];
                //     snprintf(tmp, sizeof(tmp), "0x%02x%02x, ", _this->access_buffer[i + 1], _this->access_buffer[i]);
            ...
            if ((_this->fsm_counter >= _this->tcdm_block_total) && (_this->pending_req_queue.size() == 0))
            {
                ...
                _this->num_matmul      += 1;
                _this->trace.msg("[DemocritosRedmule] Finished : %0d ns ---> %0d ns | period = %0d ns (%0d cyc) | uti = %0.3f | runtime = %0d ns | GEMM id = %0d | FMT = %0d | M-N-K = %0d-%0d-%0d | X-W-Y = 0x%5x-0x%5x-0x%5x\n",
        ...
        case FINISHED: {
            ...
            if (_this->redmule_query == NULL)
            {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][FINISHED] GRANT core and Wait for query!\n");
            } else {
        ...
        case ACKNOWLEDGE: {
            ...
            // };
            // _this->offload_grant_itf.sync(&offload_grant);
            _this->trace.msg(vp::Trace::LEVEL_TRACE,"[DemocritosRedmule][ACKNOWLEDGE] GRANT core and Done!\n");
            _this->event_enqueue(_this->fsm_event, 1);
            break;
        }
        default:
            _this->trace.fatal("[DemocritosRedmule] INVALID RedMule Status: %d\n", _this->state);
    }
}
...
void DemocritosRedmule::matmul_fp16(fp16 * z, fp16 * y, fp16 * x, fp16 * w, uint16_t m_size, uint16_t n_size, uint16_t k_size){
    for (int i = 0; i < m_size; ++i)
    {
        for (int j = 0; j < k_size; ++j)
        {
...
void matmul_uint8_uint32(uint32_t * z, uint32_t * y, uint8_t * x, uint8_t * w, uint16_t m_size, uint16_t n_size, uint16_t k_size){
    for (int i = 0; i < m_size; ++i)
    {
        for (int j = 0; j < k_size; ++j)
        {
            ...
            for (int k = 0; k < n_size; ++k)
            {
                z[i * k_size + j] += (uint32_t)x[i * n_size + k] * (uint32_t)w[k * k_size + j];
            }
        }
    }
}

void DemocritosRedmule::set_format(uint32_t compute_able){
    if (compute_able == 8) {
        this->in_elem  = 1;
        this->out_elem = 4;
    } else if (compute_able < 4) {
        this->in_elem  = 2;
        this->out_elem = 2;
    } else {
        this->in_elem  = 1;
    ...
    this->elem_size = this->in_elem;
    uint32_t out_row_bytes = this->ce_width * (this->ce_pipe + 1) * this->out_elem;
    this->out_accesses = (out_row_bytes + this->bandwidth - 1) / this->bandwidth;
    if (this->out_accesses == 0) this->out_accesses = 1;
    this->y_sub = 0;
    this->z_sub = 0;
}
